...
int main(int argc, char** argv)
{
	...
	Matrix viewM = lookat(eye, center, Vec3f(0.0, 1.0, 0.0));
	Matrix projectionM = Matrix::identity(4);
	projectionM[3][2] = -1.0f / (eye - center).norm();
	Matrix clip2screenM = viewport(width / 8, height / 8, width * 3 / 4, height * 3 / 4, depth);
	...
	for (int i = 0; i < model.nfaces(); i++) {
		...
		for (size_t i = 0; i < 3; i++)
		{
			...
			vertNormals[i] = model.normal(face[i][2]);
			...
			for (int y = ymin; y < ymax; y++)
			{
				...
				if (x >= 0 && x < width && y >= 0 && y < height && isInTriangle(screenPos2D, p))
				{
					...
					if (z > zbuffer[x + y * width])
					{
						...
						Vec3f normal = vertNormals[0] * bary.x + vertNormals[1] * bary.y + vertNormals[2] * bary.z;
						normal.normalize();
